...
std::vector<Move> Moves::possibleMoves(Gamestate::Bitboards *bitboards,
                                       bool *playingWhite)
{
  uint64_t capturablePieces = (bitboards->p | bitboards->n | bitboards->r |
                               bitboards->b | bitboards->q);
  uint64_t pieces = (bitboards->p | bitboards->n | bitboards->r | bitboards->b |
                     bitboards->q | bitboards->k);
  uint64_t occupied = (bitboards->p | bitboards->r | bitboards->n |
                       bitboards->b | bitboards->q | bitboards->k);
  uint64_t emptySpaces = ~occupied;

  uint64_t myPieces, myPawns, myRook, myBishop, myQueen, myKnight, myKing,
      theirPieces, theirPawns, theirRook, theirBishop, theirQueen, theirKnight,
      theirKing, notMyPieces, unsafe;
  ...
  unsafe =
      unsafeSpaces(&occupied, theirPawns, theirRook, theirKnight, theirBishop,
                   theirQueen, theirKing, &myKing, playingWhite);

  std::vector<Move> moves;

  if (*playingWhite == 1)
  {
    possibleWhitePawnMoves(&moves, &myPawns, &theirPawns, &capturablePieces,
                           &emptySpaces, &bitboards->enpassant, playingWhite);
    possibleWhiteCastleMoves(&moves, &occupied, &unsafe, &myKing, &myRook,
                             &bitboards->wkc, &bitboards->wqc, playingWhite);
  }
  else
  {
    possibleBlackPawnMoves(&moves, &myPawns, &theirPawns, &capturablePieces,
                           &emptySpaces, &bitboards->enpassant, playingWhite);
    possibleBlackCastleMoves(&moves, &occupied, &unsafe, &myKing, &myRook,
                             &bitboards->bkc, &bitboards->bqc, playingWhite);
  }
...
std::vector<Move> Moves::moveVectorValidator(Gamestate::Bitboards *bitboards,
                                             std::vector<Move> *moves,
                                             bool *playingWhite)
{
  ...
  for (int i = 0; i < moves->size(); i++)
  {
    ...
    Gamestate::Bitboards potentialMoveBitboards =
        Movemaker::makeMove(*bitboards, &currMove);

    uint64_t newOccupied =
        (potentialMoveBitboards.p | potentialMoveBitboards.r |
         potentialMoveBitboards.n | potentialMoveBitboards.b |
         potentialMoveBitboards.q | potentialMoveBitboards.k);

    if (*playingWhite == true)
    {
      myKing = potentialMoveBitboards.k & potentialMoveBitboards.white;
      unsafe = unsafeSpaces(
          &newOccupied, potentialMoveBitboards.p & potentialMoveBitboards.black,
          ...
          potentialMoveBitboards.k & potentialMoveBitboards.black, &myKing,
          playingWhite);
    }
    else
    {
      ...
      unsafe = unsafeSpaces(
          &newOccupied, potentialMoveBitboards.p & potentialMoveBitboards.white,
          ...
          potentialMoveBitboards.k & potentialMoveBitboards.white, &myKing,
          playingWhite);
    }
...
void Moves::addPawnMovesToMoveVector(std::vector<Move> *moves,
                                     uint64_t *toBoard, char type, int offset,
                                     bool *playingWhite)
{
  Move thisMove;
...
void Moves::addMovesToMoveVector(std::vector<Move> *moves, uint64_t *toBoard,
                                 uint64_t *fromBoard, char type,
                                 bool *playingWhite, char piece)
{
  Move newMove;
...
{
  uint64_t horizontal = (*occupied - (2 * *fromBoard)) ^
                        reverseUint64_t(reverseUint64_t(*occupied) -
                                        2 * reverseUint64_t(*fromBoard));
  uint64_t vertical =
      ((*occupied & Consts::FileMasks8[*index % 8]) - (2 * *fromBoard)) ^
      reverseUint64_t(
          reverseUint64_t(*occupied & Consts::FileMasks8[*index % 8]) -
          (2 * reverseUint64_t(*fromBoard)));

  uint64_t temp = (horizontal & Consts::RankMasks8[*index / 8]) |
                  (vertical & Consts::FileMasks8[*index % 8]);
...
{
  uint64_t diagonal =
      ((*occupied & Consts::DiagonalMasks[(*index / 8) + (*index % 8)]) -
           (2 * *fromBoard) ^
       reverseUint64_t(
           reverseUint64_t(*occupied &
                           Consts::DiagonalMasks[(*index / 8) + (*index % 8)]) -
           (2 * reverseUint64_t(*fromBoard))));
  uint64_t antidiagonal =
      ((*occupied &
        Consts::AntiDiagonalMasks[(*index / 8) + 7 - (*index % 8)]) -
           (2 * *fromBoard) ^
       reverseUint64_t(
           reverseUint64_t(
               *occupied &
  ...
  uint64_t temp =
      (diagonal & Consts::DiagonalMasks[(*index / 8) + (*index % 8)]) |
      (antidiagonal &
       Consts::AntiDiagonalMasks[(*index / 8) + 7 - (*index % 8)]);
...
void Moves::possibleWhitePawnMoves(std::vector<Move> *moves, uint64_t *myPawns,
                                   uint64_t *theirPawns,
                                   uint64_t *capturablePieces,
                                   uint64_t *emptySpaces, uint64_t *enpassant,
                                   bool *playingWhite)
{
  /*
  ...
  */
  ...
  // capture right
  temp =
      (*myPawns >> 7) & *capturablePieces & ~Consts::RANK_8 & ~Consts::FILE_A;
  addPawnMovesToMoveVector(moves, &temp, 's', -7, playingWhite);

  // capture left
  temp =
      (*myPawns >> 9) & *capturablePieces & ~Consts::RANK_8 & ~Consts::FILE_H;
  ...
  // en passant right
  temp = (*myPawns << 1) & (*theirPawns) & Consts::RANK_5 & ~Consts::FILE_A &
         *enpassant;
  addPawnMovesToMoveVector(moves, &temp, 'e', 0, playingWhite);

  // en passant left
  temp = (*myPawns >> 1) & (*theirPawns) & Consts::RANK_5 & ~Consts::FILE_H &
         *enpassant;
  addPawnMovesToMoveVector(moves, &temp, 'e', 0, playingWhite);
}

void Moves::possibleBlackPawnMoves(std::vector<Move> *moves, uint64_t *myPawns,
                                   uint64_t *theirPawns,
                                   uint64_t *capturablePeices,
                                   uint64_t *emptySpaces, uint64_t *enpassant,
                                   bool *playingWhite)
{
  uint64_t temp;
  ...
  temp =
      (*myPawns << 7) & *capturablePeices & ~Consts::RANK_1 & ~Consts::FILE_H;
  addPawnMovesToMoveVector(moves, &temp, 's', 7, playingWhite);

  temp =
      (*myPawns << 9) & *capturablePeices & ~Consts::RANK_1 & ~Consts::FILE_A;
  addPawnMovesToMoveVector(moves, &temp, 's', 9, playingWhite);
...
void Moves::possibleRookMoves(std::vector<Move> *moves, uint64_t *occupied,
                              uint64_t *notMyPieces, uint64_t r,
                              bool *playingWhite)
{
  int location = 0;
  while (r > 0)
  {
    if (r & 1)
    {
      ...
      uint64_t moveBitboard =
          hvMoves(&location, &fromBoard, occupied) & *notMyPieces;
      addMovesToMoveVector(moves, &moveBitboard, &fromBoard, 's', playingWhite,
                           'R');
    }
    r = r >> 1;
    location++;
...
void Moves::possibleBishopMoves(std::vector<Move> *moves, uint64_t *occupied,
                                uint64_t *notMyPieces, uint64_t b,
                                bool *playingWhite)
{
  int location = 0;
  while (b > 0)
  {
    if (b & 1)
    {
      uint64_t fromBoard = Consts::intToUINT.at(location);
      uint64_t moveBitboard =
          dAdMoves(&location, &fromBoard, occupied) & *notMyPieces;
      addMovesToMoveVector(moves, &moveBitboard, &fromBoard, 's', playingWhite,
                           'B');
    }
    b = b >> 1;
...
void Moves::possibleQueenMoves(std::vector<Move> *moves, uint64_t *occupied,
                               uint64_t *notMyPieces, uint64_t q,
                               bool *playingWhite)
{
  int location = 0;
  while (q > 0)
  {
    ...
    {
      uint64_t fromBoard = Consts::intToUINT.at(location);
      uint64_t moveBitboard = (dAdMoves(&location, &fromBoard, occupied) |
                               hvMoves(&location, &fromBoard, occupied)) &
                              *notMyPieces;
      addMovesToMoveVector(moves, &moveBitboard, &fromBoard, 's', playingWhite,
                           'Q');
    }
...
void Moves::possibleKnightMoves(std::vector<Move> *moves, uint64_t *notMyPieces,
                                uint64_t n, bool *playingWhite)
{
...
void Moves::possibleKingMoves(std::vector<Move> *moves, uint64_t *notMyPieces,
                              uint64_t k, uint64_t *unsafe,
                              bool *playingWhite)
{
  int location = 0;
  ...
  while (k > 0)
  {
    if (k & 1)
    {
      ...
      // if we are trying to move to an unsafe square its not allowed and dont
      // add the move
      uint64_t fromBoard = Consts::intToUINT.at(location);
      addMovesToMoveVector(moves, &moveBitboard, &fromBoard, 's', playingWhite,
                           'K');
...
uint64_t Moves::unsafeFromPawns(uint64_t *theirPawns, bool *playingWhite)
{
  if (*playingWhite)
  {
    return ((*theirPawns << 7) & ~Consts::FILE_H) |
           ((*theirPawns << 9) & ~Consts::FILE_A);
  }
  else
  {
    return ((*theirPawns >> 7) & ~Consts::FILE_A) |
           ((*theirPawns >> 9) & ~Consts::FILE_H);
  }
}

uint64_t Moves::unsafeSpaces(uint64_t *startingOccupied, uint64_t theirPawns,
                             uint64_t theirRook, uint64_t theirKnight,
                             uint64_t theirBishop, uint64_t theirQueen,
                             uint64_t theirKing, uint64_t *myKing,
                             bool *playingWhite)
{
  uint64_t unsafe;
  uint64_t temp;
  ...
  /*
  the variable temp carries all through and used for control calculations at
  each step note that it is reassigned in each section it is used in this method
  */
...
void Moves::possibleWhiteCastleMoves(std::vector<Move> *moves,
                                     uint64_t *occupied, uint64_t *unsafe,
                                     uint64_t *myKing, uint64_t *myRook,
                                     bool *wkc, bool *wqc, bool *playingWhite)
{
  std::string temp = "";
  if (*wkc == 1 && ((*occupied & Consts::WKC_INBETWEEN) == 0) &&
      ((*unsafe & Consts::WKC_INBETWEEN & Consts::WHITE_KING_START) == 0) &&
      ((*myKing & Consts::WHITE_KING_START) > 0) &&
      ((*myRook & Consts::WKC) > 1))
  {
    Move thisMove = {.fromBoard = Consts::intToUINT.at(60),
  ...
  if (*wqc == 1 && ((*occupied & Consts::WQC_INBETWEEN) == 0) &&
      ((*unsafe & Consts::WQC_INBETWEEN & Consts::WHITE_KING_START) == 0) &&
      ((*myKing & Consts::WHITE_KING_START) > 0) &&
      ((*myRook & Consts::WQC) > 1))
  {
    Move thisMove = {
...
void Moves::possibleBlackCastleMoves(std::vector<Move> *moves,
                                     uint64_t *occupied, uint64_t *unsafe,
                                     uint64_t *myKing, uint64_t *myRook,
                                     bool *bkc, bool *bqc, bool *playingWhite)
{
  std::string temp = "";
  if (*bkc == 1 && ((*occupied & Consts::BKC_INBETWEEN) == 0) &&
      ((*unsafe & (Consts::BKC_INBETWEEN | Consts::BLACK_KING_START)) == 0) &&
      ((*myKing & Consts::BLACK_KING_START) > 0) &&
      ((*myRook & Consts::BKC) > 1))
  {
    Move thisMove = {
  ...
  if (*bqc == 1 && ((*occupied & Consts::BQC_INBETWEEN) == 0) &&
      ((*unsafe & (Consts::BQC_INBETWEEN | Consts::BLACK_KING_START)) == 0) &&
      ((*myKing & Consts::BLACK_KING_START) > 0) &&
      ((*myRook & Consts::BQC) > 1))
  {
    Move thisMove = {
